...
As described in \fullref{sec:localRefFrame}, the local coordinate reference frames can also be attached to the Fanuc 210F as seen in figure \ref{fig:RefFrame}. For an overview how find the joints and links as well as the z-axis orientations see appendix \fullref{sec:jointsandlinks}. %\paragraph{Assignment of indvidual frames}
...
To wrap this up, a summary of the D-H parameters for each link as follows from figure \ref{fig:DH_Parameters_Fanuc210F} can be found in \fullref{table:DH-Parameter}.\\
...
By lifting the base frame out of the joint, parameter $d_1$ could be set to zero as well (see \fullref{fig:DH_Parameters_Fanuc210F}). For better understanding, the origin of the frame was originally kept inside the joint (see \fullref{fig:RefFrameAbstract}).
...
For further details on how to set up $R_z(\theta_i), T_z(d_i), T_x(a_i) and R_x(\alpha_i)$ and form the transformation matrix in  \ref{eq:TransformationMarix} see %chapter 1 "Homogeneous Transformations" in %"Robot manipulators: mathematics, programming, and control: the computer control of robot manipulators" 
...
With this, the complete transformation matrix can be derived in the form as described in \fullref{ForKinEq}.
The intermediate steps, that lead to the forward kinematic equations, can be found in appendix \fullref{sec:FWKinEqSteps}.
